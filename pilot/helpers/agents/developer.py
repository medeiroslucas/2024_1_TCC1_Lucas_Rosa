...
class Developer(Agent):
    ...
    def setup_environment(self, project_name: str):
    ...
    def start_coding(self,
                     ...
                     high_level_specs: str,
                     architecture: str,
                     development_plan: Dict):

        logger.info('Starting development...')

        for i in range(1, len(development_plan['plan'])+1):
            self.implement_task(i, development_plan['plan'], main_description, high_level_specs, architecture)

        message = 'Feature complete!\n'
        logger.info(message)
        print(color_green_bold(message))

    def implement_task(self, i, development_plan, main_description: str, high_level_specs: str, architecture: str):

        logger.info(color_green_bold(f'Implementing task #{i}: ')
                    + color_green(f' {development_plan[i]["description"]}\n'))
        logger.info(f'Starting task #{i} implementation...')
        ...
        conversational_agent.append_prompt_to_messages('development/task/breakdown.prompt', {
            "project_details": main_description,
            "high_level_specs": high_level_specs,
            "current_task_index": i,
            "development_tasks": development_plan,
